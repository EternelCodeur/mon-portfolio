  ...
  return (
    <AnimatedBackground className="flex items-center justify-center px-4">
      <div className="text-center max-w-6xl mx-auto">
        ...
        >
          {[
            { icon: Terminal, label: "DevOps", color: "text-green-600", bg: "from-green-500 to-green-600" },
            { icon: Code, label: "Full Stack", color: "text-blue-600", bg: "from-blue-500 to-blue-600" },
            { icon: Zap, label: "Performance", color: "text-yellow-600", bg: "from-yellow-500 to-yellow-600" }
          ].map((item, index) => (
            <motion.div
              key={item.label}
            ...
            >
              ...
              >
                <item.icon size={48} className="mb-3 relative z-10" />
              </motion.div>
              <span className="font-semibold text-gray-800 relative z-10">{item.label}</span>
            </motion.div>
          ))}
        </motion.div>
        ...
        >
          <motion.button 
            className="bg-gradient-to-r from-blue-600 to-purple-600 hover:from-blue-500 hover:to-purple-500 text-white px-8 py-4 rounded-full text-lg font-semibold shadow-lg hover:shadow-xl transform transition-all duration-300 relative overflow-hidden group"
